...
export async function render() {
	...
	// Group all querySelectors
	const queryElements = todos.map(todo => ({
		id: todo.id,
		deleteButton: document.getElementById(`deleteButton-${todo.id}`),
		editButton: document.getElementById(`editButton-${todo.id}`),
		checkbox: document.getElementById(`toggleCheckbox-${todo.id}`)
	}));

	// Group all eventListeners
	queryElements.forEach(({ id, deleteButton, editButton, checkbox }) => {
		// Delete event
		if (deleteButton) {
		...
		if (editButton) {
			editButton.addEventListener('click', () => {
				...
				// Save button hidden if input blank
				...
				saveBtn.onclick = async () => {
					...
					if (newTitle && newDueDate) {
						const success = await editTodo(id, { ...todo, title: newTitle, dueDate: newDueDate });
						if (addBtn) addBtn.style.display = '';
						render();
					}
				};
